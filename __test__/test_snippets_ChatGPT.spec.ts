...
test('reference_in_text_literal', async() => {
  ...
  expect(genBaseSnippet_ChatGPT(mock_textLiteral, map)).not.toContain(map.get("@elemento")); 
...
test('reference_in_input_ref', async() => {
  ...
  let map= new Map<string,string>;
  map.set( "@elemento","fuego")
  expect(genBaseSnippet_ChatGPT(mock_InputRef, map)).toBe(map.get("@elemento"));
...
test('reference_in_combination_trait', async() => {
  ...
  expect(genBaseSnippet_ChatGPT(mock_CombTrait, map)).toContain(""); 
  // Check that the InputRef are changed
...
test('reference_in_composer', async() => {
  let map= new Map<string,string>;
  map.set( "@elemento","fuego")
  expect(genAsset_ChatGPT(mock_composer,map)[0]).toBe("fuego");
  expect(genAsset_ChatGPT(mock_composer,map)[1]).toBe("fuego");
})
*/
